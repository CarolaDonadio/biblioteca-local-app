...
<div class="pub-contenido">
  <div style="margin-bottom:1.5em;">
    <a href="<?= base_url('catalogo') ?>" style="text-decoration:none;color:var(--azul-primario, #2563eb);font-weight:600;">← Volver al catálogo</a>
  </div>
  ...
  <div class="tarjeta" style="padding:2em;display:grid;grid-template-columns:repeat(auto-fit, minmax(260px, 1fr));gap:2em;">
    ...
    <div style="text-align:center;background:#f3f4f6;border-radius:var(--radio, 8px);padding:2em;display:flex;align-items:center;justify-content:center;min-height:300px;">
      <?php if (!empty($libro['portada_url'])): ?>
        <img class="lazy" data-src="<?= base_url('uploads/' . $libro['portada_url']) ?>" alt="<?= esc($libro['titulo']) ?>" style="max-width:100%;max-height:360px;object-fit:contain;border-radius:var(--radio);">
      <?php else: ?>
    ...
    <div style="display:flex;flex-direction:column;justify-content:space-between;">
      <div>
        ...
        <h1 style="margin:.4em 0 .2em 0;"><?= esc($libro['titulo']) ?></h1>
        
        <p style="color:var(--gris-texto);font-size:1.1rem;margin-bottom:1.5em;">
          Autor: <strong><?= esc($libro['autor']) ?></strong>
        </p>
        ...
        <div class="tarjeta" style="padding:1em;background:#fafafa;margin-bottom:1.5em;display:grid;grid-template-columns:repeat(auto-fit, minmax(120px, 1fr));gap:1em;font-size:.9rem;">
          <div>
            ...
            <strong><?= esc($libro['isbn'] ?? 'N/A') ?></strong>
          </div>
          <div>
            <span style="color:var(--gris-texto);display:block;">Editorial</span>
            <strong><?= esc($libro['editorial'] ?? 'No especificada') ?></strong>
          </div>
          <div>
            <span style="color:var(--gris-texto);display:block;">Año</span>
            <strong><?= esc($libro['anio'] ?? 'N/D') ?></strong>
          </div>
          <div>
            <span style="color:var(--gris-texto);display:block;">Disponibilidad</span>
            <?php if (!empty($libro['disponible']) && ($libro['cantidad'] ?? 0) > 0): ?>
              <span style="color:#166534;font-weight:bold;">Disponible (<?= $libro['cantidad'] ?> un.)</span>
            <?php else: ?>
              <span style="color:#991b1b;font-weight:bold;">Agotado</span>
            <?php endif; ?>
          </div>
        </div>
        ...
        <div style="margin-bottom:1.5em;">
          ...
          <p style="color:var(--gris-texto);line-height:1.6;font-size:.95rem;">
            <?= esc($libro['sinopsis'] ?? 'Sin descripción disponible para este libro.') ?>
          </p>
        </div>
      </div>
      ...
      <div>
        ...
        <?php else: ?>
          <a href="<?= base_url('socio/login') ?>" class="btn" style="display:block;text-align:center;text-decoration:none;">Ingresá a tu cuenta para reservar</a>
        <?php endif; ?>
      </div>
